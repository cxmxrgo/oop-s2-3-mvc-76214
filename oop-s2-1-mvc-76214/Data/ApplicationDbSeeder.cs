...
public static class ApplicationDbSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        {
            var firstEnrolmentId = await context.CourseEnrolments.OrderBy(x => x.Id).Select(x => x.Id).FirstAsync();
            context.AttendanceRecords.AddRange(
                new AttendanceRecord { CourseEnrolmentId = firstEnrolmentId, WeekNumber = 1, AttendanceDate = new DateTime(2026, 9, 7), Present = true },
                new AttendanceRecord { CourseEnrolmentId = firstEnrolmentId, WeekNumber = 2, AttendanceDate = new DateTime(2026, 9, 14), Present = true });
        }
